...
const Home = ({ query, items: allFlags, continents = [] }) => {
  ...
  // get continents label
  const continentsObject = {}
  filtersValues.continents.forEach(continent => {
    continentsObject[continent] = continents[continent]
  })

  filtersValues.continents = continentsObject
